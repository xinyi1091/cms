...
$sql = "SELECT id,title,postdate,enddate,i.catid,c.catname,i.areaid,a.areaname,thumb,i.description,i.phone FROM {$table}info AS i LEFT JOIN {$table}category AS c ON i.catid=c.catid LEFT JOIN {$table}area AS a ON a.areaid=i.areaid WHERE is_check=1 $cat_sql $area_sql ORDER BY postdate DESC limit $pager[start],$pager[size]";
$res = $db->query($sql);
$info = array();
